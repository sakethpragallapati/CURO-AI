...
def process_curo_query(user_query: str, demography: dict = None, user_id: str = None) -> dict:
    ...
    kg_prompt = PromptTemplate.from_template(
        ...
        "ACADEMIC LITERATURE:\n{text}\n\n"
        "INSTRUCTION: Extract exact logical relationships (Triplets). You MUST create a complex, interconnected web, NOT a simple star graph. "
        "Rules for relationships:\n"
        "1. Link {topic} to patient symptoms (PRESENTS_WITH), Risk Factors (INCREASES_RISK), Treatments (TREATED_WITH), and Differentials (DIFFERENTIAL_DIAGNOSIS).\n"
        "2. CRITICAL - DIRECTIONALITY: Relationships MUST make logical sense when read as 'Subject -> Relation -> Object'. Always use ACTIVE VOICE.\n"
        "   - Ensure the subject actively performs the relation on the object (e.g., [Entity A] -[TRIGGERS/CAUSES/DIAGNOSES]-> [Entity B], avoiding passive forms like TRIGGERED_BY).\n"
        "3. CRITICAL - INTERLINKING: You MUST also interlink the peripheral nodes to each other! For example:\n"
        "   - Link specific treatments directly to the exact symptoms they relieve.\n"
        "   - Link differential diagnoses directly to the shared symptoms they present with.\n"
        "DO NOT create a simple hub-and-spoke graph. Ensure there are many cross-connections between symptoms, treatments, and differentials.\n"
