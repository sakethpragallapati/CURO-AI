...
def process_curo_query(user_query: str, demography: dict = None, user_id: str = None) -> dict:
    ...
    kg_prompt = PromptTemplate.from_template(
        ...
        "2. CRITICAL: You MUST also interlink the peripheral nodes to each other! For example:\n"
        "   - Link specific Treatments to the exact Symptoms they relieve (e.g., Morphine -[ALLEVIATES]-> Abdominal Pain).\n"
        "   - Link Differentials to the shared symptoms they cause (e.g., Ovarian Cystitis -[PRESENTS_WITH]-> Abdominal Pain).\n"
